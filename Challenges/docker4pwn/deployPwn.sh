...
# to deploy pwn challenges in docker
## for each directory
## e.g.
### ./docker4pwn100/
####  |
...
default_cnt=10001
dk_expose_port=10000
port_cnt=$default_cnt
subDirName="docker4pwn*"

clear

while [ 1 ]; do
	echo -e "+-----------------------------------------------------+"
	echo -e "|           CTF Pwn Challenges Deployment             |"
	echo -e "+-----------------------------------------------------+"
	...
	if [ $opt == 'q' ]; then
		...
		for pwnDir in $(ls -d $subDirName); do
			cd $pwnDir
			docker build -t $pwnDir .
			cd ..
		done
		clear
	elif [ $opt == '2' ]; then
		for pwnDir in $(ls -d $subDirName); do
			docker run -d -p $port_cnt:$dk_expose_port $pwnDir
		...
		docker run -d -p $some_port:$dk_expose_port $some_image
	elif [ $opt == 'b' ]; then
		docker container ls -a | grep "$subDirName"
